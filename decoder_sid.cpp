...
#include <fstream> //For using C64 ROMs
#include <string>
...
//ROM loader, modified from the one in the libsidplayfp example code.
char* loadRom(std::string path, size_t romSize)
{
    char* buffer = 0;
    ...
    if (is.good())
    {
        ...
        qDebug("DecoderSID: Loaded ROM: %s (%db)", path.c_str(), (int)romSize);
    }
    is.close();
    return buffer;
}
...
bool DecoderSID::initialize()
{
    ...
    QString default_rom_path = Qmmp::configDir() + "sidplayfp";
    std::string kernal_path = std::string(qPrintable(settings.value("rom_path", default_rom_path).toString())) + "/kernal.bin";
    std::string basic_path = std::string(qPrintable(settings.value("rom_path", default_rom_path).toString())) + "/basic.bin";
    std::string chargen_path = std::string(qPrintable(settings.value("rom_path", default_rom_path).toString())) + "/chargen.bin";

    settings.endGroup();
    
    //load optional roms for more accurate and compatible sid emulation
    char *kernal = loadRom(kernal_path, 8192);
    char *basic = loadRom(basic_path, 8192);
    char *chargen = loadRom(chargen_path, 4096);
    m_player->setRoms((const uint8_t*)kernal, (const uint8_t*)basic, (const uint8_t*)chargen);
    delete [] kernal;
    delete [] basic;
    delete [] chargen;
